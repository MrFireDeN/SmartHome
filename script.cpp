...
QVector<QVector<QString>> Script::getSmartSpeaker() {
    // Implement the function logic here
    morning = {
        "Голосовое управление - просмотр видеопотока с камер, включая различные режимы обзора.",
        "Оповещение о зафиксированном движении на улице/в доме при выходе.",
        "Воспроизведение аудиосообщений, например, напоминаний о запланированных событиях."
    };
    day = {
        "Двусторонняя аудио/видеосвязь через колонку для общения с посетителями.",
        "Дистанционный доступ через приложение - просмотр трансляций, запись видео.",
        "Наблюдение за детьми/домашними животными с возможностью давать голосовые команды.",
    };
    evening = {
        "Автоматический переход в ночной режим активного патрулирования по расписанию.",
        "Голосовые команды для включения режима \"Охрана\", активации датчиков.",
        "Интеграция с \"Умным домом\" - включение света и анонсирование тревоги голосом при срабатывании датчиков."
    };
    return {morning, day, evening};
}
...
QVector<QVector<QString>> Script::getSocket() {
    // Implement the function logic here
    morning = {
        "Автоматическое включение кофеварки/чайника к назначенному времени пробуждения.",
        "Розетка для фена/утюга активируется при входе в ванную/гардеробную.",
        "Включение зарядки для электрических зубных щеток по будильнику."
    };
    day = {
        "Отключение питания неиспользуемых в дневное время бытовых приборов для экономии энергии.",
        "Включение зарядки для роботов-пылесосов во время вашего отсутствия дома.",
        "Автоматическое включение подогрева полов в определенное время.",
    };
    evening = {
        "Включение арома-ламп, светильников и других приборов для создания уютной атмосферы.",
        "Если дом зарядка аккумуляторов садовой техники после 20:00.",
        "Отключение питания от зарядных устройств после завершения зарядки гаджетов."
    };
    return {morning, day, evening};
}
